...
export class MailDetailsComponent implements OnInit, OnDestroy
{
    ...
    resendEmail(event, mail: Mail): void
    {
        // resend this particular Email
        this._mailService.resendMail(mail)
        .pipe(first())
            .subscribe(
                data => {
                    //this.damagedAssetTaskID = +data;
                ...
                error => {
                    ...
                    if (error.status == 0) {
                        alert("HttpErrorResponse, Unknown Error. Please Check Network Status");
                    }
